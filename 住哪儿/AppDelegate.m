...
- (void)application:(UIApplication *)application didReceiveLocalNotification:(UILocalNotification *)notification{
    ...
    NSKTLog(@"applicationState.rawValue: %zd", application.applicationState);
    ...
    if (application.applicationState == UIApplicationStateActive) {
        ...
        // 后台进入前台
        NSKTLog(@"执行后台进入前台对应的操作");
        NSKTLog(@"*****%@", notification.userInfo);
...
-(BOOL)application:(UIApplication *)application openURL:(nonnull NSURL *)url sourceApplication:(nullable NSString *)sourceApplication annotation:(nonnull id)annotation{
    NSKTLog(@"calling application bundle id: %@",sourceApplication);
    NSKTLog(@"url shceme:%@",[url scheme]);
    NSKTLog(@"参数:%@",[url query]);
