...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        NSKTLog(@"Warning: There seem to be no columns in this set.");
    }
...
- (BOOL)nextWithError:(NSError **)outErr {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        NSKTLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [_parentDB databasePath]);
        NSKTLog(@"Database busy");
        if (outErr) {
        ...
        NSKTLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
        ...
        // wtf?
        NSKTLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([_parentDB sqliteHandle]));
        if (outErr) {
            *outErr = [_parentDB lastError];
        }
    }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    NSKTLog(@"Warning: I could not find the column named '%@'.", columnName);
